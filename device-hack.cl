...
__kernel void hack(__global HackParams * params, __global HackResults * hackResults, __global int * mem) {
  for (uint i = 0; i < params->blockSize; i++) {
//    uint idx = (uint) params->memLength + get_global_id(0) * params->blockSize + i;
